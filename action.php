...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>ประเภท</h4></a></li>
	";
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>ยี่ห้อ</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
				...
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$.$pro_price.00
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>เพิ่มในลงรถเข็น</button>
								</div>
							</div>
						</div>	
			";
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>เพิ่มสินค้าลงในรถเข็นแล้ว ช็อปปิ้งต่อ ..!</b>
				</div>
			";//not in video
		} else {
			...
			echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>ขออภัยค่ะ..! กรุณาลงทะเบียนก่อน จากนั้นคุณสามารถเพิ่มผลิตภัณฑ์ในรถเข็นของคุณค่ะ</b>
					</div>
				";
		}
	...
	if($count > 0){
		...
		if(isset($_POST["cart_checkout"])){
			echo "<div class='row'>
				...
				<div class='col-md-4'>
					<h1>ทั้งหมด $$total_amt</h1>
				</div>";
		}
		echo '
		
				<form action="https://www.paypal.com/th/signin" method="post">
	...
	if($run_query){
		echo "
			<div class='alert alert-danger'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>สินค้าถูกลบออกจากรถเข็นแล้ว กรุณาช็อปปิ้งต่อ..!</b>
			</div>
		";
	}
}
